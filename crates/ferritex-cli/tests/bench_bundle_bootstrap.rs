...
#[test]
fn partition_bench_appendix_chapters_output_identity_across_jobs_1_and_jobs_4() {
    let bench_fixtures = bench_fixtures_root();
    let input_fixture = bench_fixtures
        .join("corpus")
        .join("partition-book")
        .join("appendix_chapters.tex");
    let cases = [1, 4]
        .into_iter()
        .map(|jobs| BenchCase {
            name: format!("partition-partition-book-appendix_chapters-jobs{jobs}"),
    ...
    assert!(
        report.failures.is_empty(),
        "appendix_chapters partition bench compilation failed: {:?}",
        report.failures
    );
    ...
    assert_eq!(
        ...
        "appendix_chapters partition bench output should be identical across jobs=1 and jobs=4"
    );
}
