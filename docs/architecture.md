...
| 最終更新日 | 2026-03-17 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.33, [domain_model.md](domain_model.md) v0.1.34 |
...
- commit 順序: `CommitBarrier` は `(passNumber, stageOrder, partitionId)` の total order で結果を適用する (`REQ-FUNC-031`, `REQ-FUNC-032`)。各段階は `StageCommitPayload` の対応サブタイプで commit payload を表現し、`stageOrder` は以下の順序に固定する:
...
3. `CompileJobService` が正規化済み `RuntimeOptions` と `ExecutionPolicy` を受けて `CompilationJob` を生成し、`Incremental Compilation` がフル or 差分方針と stable `partitionId` / `PartitionLocator` 付きの `DocumentPartitionPlan` を決める。パーティション境界は前 pass で確定した `DocumentState.toc` を primary source とし、cold start または TOC 未確定時は `Incremental Compilation` 内の sectioning pre-scan が検出した sectioning command の `SourceSpan` と `DependencyGraph` のファイル間依存から暫定 plan を生成する。cache 破損が報告された場合、`Incremental Compilation` は dependency graph を invalidation / watch-set refresh の補助にだけ使い、出力生成はフルコンパイルへ fallback させる。
