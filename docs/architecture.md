...
| 最終更新日 | 2026-03-15 |
| ステータス | 提案 |
| 入力 | [requirements.md](requirements.md) v0.1.17, [domain_model.md](domain_model.md) v0.1.19 |
...
| QA-08 | 構文エラーまたは外部実行拒否が発生する | compile / watch / lsp の任意入口 | ファイル名・行番号・文脈・可能なら修正候補を返す | 必須フィールド充足率 100% (`REQ-NF-010`) |
...
- commit 順序: `CommitBarrier` は `(passNumber, stageOrder, partitionId)` の total order で結果を適用し、`stageOrder` は `macro/session delta -> document/reference/bibliography state -> layout/page-number merge -> artifact emission/cache metadata` に固定する
...
| `LspCapabilityService` | `initialize` 応答で advertise する `textDocumentSync` / `completionProvider` / `codeActionProvider` と definition / hover などの optional provider を構築し、push diagnostics を含む LSP 配線を担う |
| `PreviewSessionService` | preview session の発行・再発行・失効、`PreviewTarget` owner 管理、閲覧位置保存、PDF 更新通知、`ExecutionPolicy.previewPublication` に照らした publish 可否判定と再適用を行う |
...
| `PreviewTransport` | session ごとの PDF document endpoint と revision events endpoint を提供 | loopback のみへ bind し、`GET /preview/{sessionId}/document` と `WS /preview/{sessionId}/events` を公開する。bind / publish は `PreviewSessionService` が `ExecutionPolicy.previewPublication` と `PreviewTarget` 一致を確認した場合にだけ実行する |
...
1. `NativeFileWatcher` が変更を集約し、`RecompileScheduler` が `PendingChangeQueue` へ coalesce する。
2. `RecompileScheduler` は active job 実行中の新イベントを保留し、`WorkspaceJobScheduler` は workspace 単位の排他と publish 順序を保証する。
3. compile 完了後、`PreviewSessionService` が session owner である `PreviewTarget` と active job の target を照合し、`ExecutionPolicy.previewPublication` に照らして publish 可否を判定する。許可された場合だけ閲覧位置を再適用したうえで `PreviewTransport` に loopback 上の session endpoint への target 付き最新 revision 配信を指示する。target 変更または process restart 後は session を再発行する。
...
3. diagnostics / codeAction / completion / definition / hover は全て `LiveAnalysisSnapshot` を唯一の入力として参照し、active compile/watch job の完了を待たない。
...
| --- | --- | --- |
| Text Document Sync | Must。`didOpen` / `didChange` / `didClose` を受け付け、push diagnostics の前提を提供する | 常に advertise |
| Code Action | Must。代表的な構文エラー修正候補を返す | 常に advertise |
| Completion | Must。command / environment / label / citation を返す | 常に advertise |
| Definition | Should。macro / label / citation に対応する | definition provider 有効時のみ advertise |
...
- **計測対象**: compile / watch / lsp の診断と拒否メッセージ
- **閾値**: ファイル名・行番号・要約・文脈 snippet を 100% 付与し、修正候補を出せるケースでは suggestion を付与する
- **計測方法**: golden diagnostics テストで `file`, `line`, `message`, `context`, `suggestion?` の各フィールドを検証する
- **違反時のアクション**: build を失敗させ、`DefinitionProvenance` / `SourceSpan` の欠落箇所を修正する
...
- `PreviewSessionService` は preview publish 前に `ExecutionPolicy.previewPublication` と `PreviewTarget` 一致を確認し、許可された場合だけ `PreviewTransport` に loopback bind 済み endpoint への配信を指示する
