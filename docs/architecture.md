...
| 最終更新日 | 2026-03-15 |
| ステータス | 提案 |
| 入力 | [requirements.md](requirements.md) v0.1.16, [domain_model.md](domain_model.md) v0.1.18 |
...
| `PreviewSessionService` | preview session の閲覧位置保存、PDF 更新通知、`ExecutionPolicy` に照らした publish 可否判定、再適用を行う |
...
| `PreviewTransport` | session ごとの PDF document endpoint と revision events endpoint を提供 | loopback のみへ bind し、`GET /preview/{sessionId}/document` と `WS /preview/{sessionId}/events` を公開する。bind / publish は `PreviewSessionService` が `ExecutionPolicy` で許可した場合にだけ実行する |
...
3. compile 完了後、`PreviewSessionService` が `ExecutionPolicy` に照らして publish 可否を判定し、閲覧位置を再適用したうえで `PreviewTransport` に loopback 上の session endpoint への最新 revision 配信を指示する。
...
- `ExecutionPolicy` は compile / watch / lsp / preview publish path で共通化し、preview では bind 先、公開可否、配信対象 artifact の選別を担う
- `FileAccessGate` は filesystem の read / write / readback だけを仲介し、socket accept/connect と `PreviewSession` の in-memory state は扱わない
- `PreviewSessionService` は preview publish 前に `ExecutionPolicy` を確認し、許可された場合だけ `PreviewTransport` に loopback bind 済み endpoint への配信を指示する
...
| Preview transport | ブラウザ cache や stale session が古い PDF を見せる可能性がある | session ごとに revision を付与し、`no-store` と再接続時の session 再発行を徹底する |
