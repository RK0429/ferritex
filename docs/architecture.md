...
| 最終更新日 | 2026-03-18 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.39, [domain_model.md](domain_model.md) v0.1.37 |
...
| QA-10 | bundle-only 環境で baseline 文書群をコンパイルする | `FTX-ASSET-BUNDLE-001` のみ、TeX Live 非導入、同一入力・同一マシン | Asset Bundle だけで `FTX-CORPUS-COMPAT-001/layout-core` を起動コンパイルできる | `layout-core` の baseline 文書群が全件コンパイル成功 (`REQ-FUNC-046`) |
...
- **計測対象**: `FTX-ASSET-BUNDLE-001` のみを導入した環境での `FTX-CORPUS-COMPAT-001/layout-core` 起動コンパイル
- **閾値**: TeX Live 非導入の bundle-only 環境で `layout-core` の baseline 文書群が全件コンパイル成功する
- **計測方法**: `ferritex-bench` で `FTX-CORPUS-COMPAT-001/layout-core` を対象に、同一入力・同一マシン上で `FTX-ASSET-BUNDLE-001` だけを mount した sandbox を構築し、TeX Live / host TEXMF を無効化した状態でコンパイル成功可否と起動診断を検証する
...
| 適合度関数の要件トレーサビリティ | 充足 | §10.3 差分コンパイル正確性に `REQ-FUNC-030`、§10.16 Asset Bundle bootstrap に `REQ-FUNC-046` を紐づけ済み |
