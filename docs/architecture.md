...
| 最終更新日 | 2026-03-17 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.30, [domain_model.md](domain_model.md) v0.1.31 |
...
- commit 順序: `CommitBarrier` は `(passNumber, stageOrder, partitionId)` の total order で結果を適用する (`REQ-FUNC-031`, `REQ-FUNC-032`)。`stageOrder` は以下の順序に固定する:
  ...
  `partitionId` は `DocumentPartitionPlanner` が安定に発行する
...
4. `Parser` は citation 系参照（`\cite`）と label 系参照（`\ref`/`\pageref`）を分けて生成し、`Bibliography Integration` が `.bbl` 由来の `CitationTable` / `BibliographyEntry` を構築する。`Typesetting` は `CrossReferenceTable` を label 系に、`BibliographyState` を参考文献リスト組版に使い、その後 `Graphics` / `Font` / `PDF` が pipeline を実行する。文書パーティション単位並列化（`REQ-FUNC-032`）が有効な場合、各パーティションの組版結果は `DocumentLayoutFragment` として生成され、`PaginationMergeCoordinator` がページオフセットと参照整合性を保ったまま統合する（`domain_model.md` §3.4 参照）。
...
5. compile 完了後、`PreviewSessionService` が session owner である `PreviewTarget` と active job の target を照合し、`ExecutionPolicy.previewPublication` に照らして publish 可否を判定する。許可された場合だけ閲覧位置を再適用したうえで `PreviewTransport` に loopback 上の session endpoint への target 付き最新 revision 配信を指示する。session 失効・target 不一致・policy 拒否時は `SessionErrorResponse`（`errorKind`, `sessionId`, `recoveryInstruction`）を返す (`REQ-NF-010`)。
6. 新しい依存グラフに基づき監視対象集合を差し替える。

### 7.3 `lsp`

1. `LSP Adapter` が workspace 既定値とクライアント設定を `RuntimeOptions` に正規化しつつ (`REQ-FUNC-045`)、未保存テキストを `OpenDocumentStore` に保持する。
