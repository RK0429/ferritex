...
| 最終更新日 | 2026-03-13 |
| ステータス | 提案 |
| 入力 | [requirements.md](requirements.md) v0.1.14, [domain_model.md](domain_model.md) v0.1.16 |
...
| QA-04 | 外部コマンド実行や許可外ファイルアクセスを試みる | `compile` / `watch` / `lsp` の任意入口 | 既定で拒否し、許可時も job 単位の上限を適用する | 外部実行経路 0、許可外 I/O 経路 0 (`REQ-NF-005`, `REQ-NF-006`) |
| QA-05 | 標準的な論文文書を 3 OS で処理する | Host Font Catalog overlay を無効化した再現ビルド | 同一 PDF を生成する | メタデータを除きバイト同一 (`REQ-NF-008`) |
| QA-06 | プレビュー接続中に再コンパイルが完了する | 閲覧位置を保持した preview session が存在する | PDF を更新し、閲覧位置を復元する | 更新 1 秒未満 (`REQ-FUNC-040`) |
...
| Application | job orchestration、scheduler、snapshot 構築、use case 統合 | CompileJobService, RecompileScheduler, LiveAnalysisSnapshotFactory, PreviewSessionService |
| Domain | TeX 処理の本体、決定的状態遷移、差分統合、PDF 射影 | Parser, Typesetting, Incremental Compilation, Asset Runtime, Graphics, PDF, Font |
...
graph LR
    ...
    AnalysisSvc --> Parser
    AnalysisSvc --> Bibliography
    AnalysisSvc --> Incremental
    PreviewSvc --> Pdf
...
| コンポーネント | 責務 | 主な公開契約 |
| --- | --- | --- |
| Parser & Macro Engine | 字句解析、マクロ展開、条件分岐、パッケージ読み込み、job/pass 状態管理 | `CompilationSession`, `DocumentStateDelta`, `GraphicsCommandStream`, `DependencyEvents` |
| Bibliography Integration | `.bbl` 読み込み、`BblSnapshot` / `CitationTable` 構築、`\cite` 解決、参考文献ブロック生成 | `BibliographyState`, `CitationTable`, `BibliographyBlock` |
| Typesetting Engine | 行分割、ページ分割、display math、フロート、脚注、目次/索引、参考文献リストのレイアウト | `PageBox`, `FloatPlacement`, `LayoutFragment` |
| Incremental Compilation | 依存グラフ、影響範囲判定、再利用/再構築統合、固定点反復、文書パーティション統合 | `RecompilationScope`, `CompilationMergePlan`, `PaginationMergeResult`, `CompilationSnapshot` |
...
| サービス | 責務 |
| --- | --- |
| `CompileJobService` | `compile` / `watch` / `lsp` 共通の use case を起動し、runtime options を正規化する |
| `WorkspaceJobScheduler` | workspace ごとの排他制御、pending change の coalesce、compile と preview 更新の順序保証 |
| `LiveAnalysisSnapshotFactory` | `OpenDocumentBuffer` と安定化済み compile state から LSP 専用の immutable projection を構築する |
| `LspCapabilityService` | `initialize` 応答で advertise する capability を構築し、push diagnostics / codeAction / completion / definition / hover を配線する |
| `PreviewSessionService` | preview session の閲覧位置保存、PDF 更新通知、再適用を行う |
| `CacheMaintenanceService` | invalidation、integrity check、LRU eviction、破損検知時のフルコンパイル fallback を統括する |
| `ExecutionPolicyFactory` | 入口ごとの差を吸収し、共通 `ExecutionPolicy` を生成する |
...
| `PreviewTransport` | PDF 配信と更新イベント通知 | 既定は loopback 限定。HTTP + WebSocket は暫定選択 |
...
トップレベル crate はレイヤ境界として使い、ドメイン境界はまず `ferritex-core` 内の垂直モジュールで表現する。`parser`, `bibliography`, `typesetting`, `incremental`, `graphics`, `font`, `pdf`, `assets`, `kernel` を一次単位とし、次の条件を満たした場合だけ独立 crate へ昇格する。
...
1. `CLI Adapter` が `RuntimeOptions` を生成する。
2. `ExecutionPolicyFactory` が workspace と option から共通ポリシーを構築する。
3. `CompileJobService` が `CompilationJob` を生成し、`Incremental Compilation` がフル or 差分方針を決める。
4. `Parser` が `Bibliography Integration` と協調して `BibliographyState` / `CitationTable` を更新し、その後 `Typesetting` / `Graphics` / `Font` / `PDF` が pipeline を実行する。
5. `OutputArtifactRegistry` は active `JobContext` に対する in-memory registry として更新し、必要に応じて監査用 manifest を出力する。
6. `CacheMaintenanceService` が dependency graph と cache metadata を更新し、PDF / SyncTeX / diagnostics を出力する。

### 7.2 `watch` / preview

1. `NativeFileWatcher` が変更を集約し、`WorkspaceJobScheduler` が coalesce する。
2. active job 実行中は新イベントを `PendingChangeQueue` に蓄積する。
3. compile 完了後、`PreviewSessionService` が最新 PDF を配信し、閲覧位置を再適用する。
4. 新しい依存グラフに基づき監視対象集合を差し替える。

### 7.3 `lsp`

1. `LSP Adapter` が未保存テキストを `OpenDocumentStore` に保持する。
2. `LiveAnalysisSnapshotFactory` が最新 compile state の安定化済み projection を受け取り、`OpenDocumentBuffer` と合成した LSP 専用 `LiveAnalysisSnapshot` を構築する。
3. diagnostics / codeAction / completion / definition / hover は全て `LiveAnalysisSnapshot` を唯一の入力として参照する。
4. diagnostics は `textDocument/publishDiagnostics` を既定とし、pull diagnostics は adapter 拡張として別契約で扱う。
5. `LspCapabilityService` が `initialize` 応答で capability を advertise し、build 設定に応じて optional provider を制御する。

#### LSP capability matrix

| 機能 | 対応方針 | advertise 条件 |
| --- | --- | --- |
| Diagnostics | Must。push を既定とする | 常に advertise |
| Code Action | Must。代表的な構文エラー修正候補を返す | 常に advertise |
| Completion | Must。command / environment / label / citation を返す | 常に advertise |
| Definition | Should。macro / label / citation に対応する | 実装時は advertise |
| Hover | Could。`HoverDocCatalog` が有効な build のみ | provider 有効時のみ advertise |
...
| Preview transport | HTTP で PDF 配信 + WebSocket で invalidation / view restore | PDF 配信の単純さと低遅延通知を両立する | `REQ-FUNC-040` の未確定事項のため adapter で差し替え可能にする |
...
- **計測対象**: 標準文書 100 件に対する pdfLaTeX とのレイアウト一致率
- **閾値**: 行分割・ページ分割ベースで 95% 以上一致
- **計測方法**: `ferritex-bench` で pdfLaTeX 出力との差分を比較する
- **違反時のアクション**: どのコンテキストで差が生じたかを `parser` / `typesetting` / `font` / `pdf` 単位で切り分ける
...
- 主要メトリクス: `compile_full_ms`, `compile_incremental_ms`, `lsp_diagnostics_ms`, `preview_publish_ms`, `cache_hit_ratio`, `passes_used`, `denied_file_access_total`, `denied_process_spawn_total`
- アラート閾値:
...
- `ExecutionPolicy` を compile / watch / lsp で共通化する
- `FileAccessGate` がすべての read / write / readback を仲介する
- preview transport は loopback bind を既定とし、外部公開しない
...
- trusted artifact は active `JobContext` にのみ有効とし、job 完了または process restart で無効化する
...
| 項目 | リスク | 対応 |
| --- | --- | --- |
| pdfLaTeX 比 100x | 成功基準は高いが、詳細要件への落ち方がまだ粗い | `ferritex-bench` を最優先で整備し、PoC で律速を確認する |
| `LiveAnalysisSnapshot` の不変性 | shallow copy 実装だと LSP が中途状態を観測する | `CompilationSnapshot` と分離し、immutable projection だけを保持する |
| Preview transport | `REQ-FUNC-040` の最終プロトコルが未確定 | `PreviewTransport` port を切り、HTTP/WebSocket を暫定採用する |
...
| 観点 | 状態 | 補足 |
| --- | --- | --- |
| 品質特性シナリオ定義 | 充足 | QA-01 〜 QA-06 に整理済み |
...
| 未確定事項の隔離 | 一部充足 | preview protocol と benchmark 実現性は今後の確認が必要 |
