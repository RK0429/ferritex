...
| 最終更新日 | 2026-03-18 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.41, [domain_model.md](domain_model.md) v0.1.39 |
...
| サービス | 責務 |
| --- | --- |
| `CompileJobService` | 各入口で正規化済みの `RuntimeOptions` を受け取り、`compile` / `watch` / `lsp` 共通の use case を起動する。`RuntimeOptions.traceFontTasks=true` の場合はフォント処理ステージに public NDJSON `FontTaskTrace`（`schemaVersion="ferritex.fontTaskTrace.v1"`）の `stderr` 出力を有効化する |
...
1. `CLI Adapter` が CLI フラグを `RuntimeOptions` に正規化する (`REQ-FUNC-042`)。`--trace-font-tasks` は `RuntimeOptions.traceFontTasks` として保持する。`--format json` は compile adapter の標準出力表現だけを切り替え、`RuntimeOptions` や compile pipeline の意味論には混ぜない。
...
3. `CompileJobService` が正規化済み `RuntimeOptions` と `ExecutionPolicy` を受けて `CompilationJob` を生成し、`Incremental Compilation` がフル or 差分方針と stable `partitionId` / `PartitionLocator` 付きの `DocumentPartitionPlan` を決める。パーティション境界は前 pass で commit 済みの `DocumentStateView.toc` を primary source とし、cold start または TOC 未確定時は `Incremental Compilation` 内の sectioning pre-scan が検出した sectioning command の `SourceSpan` と `DependencyGraph` のファイル間依存から暫定 plan を生成する。cache 破損が報告された場合、`Incremental Compilation` は dependency graph を invalidation / watch-set refresh の補助にだけ使い、出力生成はフルコンパイルへ fallback させる。`--trace-font-tasks` 指定時は public automation output として `schemaVersion="ferritex.fontTaskTrace.v1"` を持つ `FontTaskTrace` NDJSON を `stderr` に出力し、warm-cache hit 経路でも検証用の `FontTaskTrace` センチネル（`fontTaskId="font-load-cache-hit"`、`fontAsset="builtin:font-cache-hit"`）を `stderr` に 1 件 emit する。センチネルは `--trace-font-tasks` が warm 経路でも有効であることを示す presence marker であり、`REQ-FUNC-033` の overlap 判定（独立タスクが 2 件以上並走すること）には寄与しない。overlap は `--no-cache` を指定した cold 経路で検証する。
...
6. `PDF Renderer` / `SyncTeX` が成果物を出力し、`CompileJobService` が diagnostics を集約する。`CacheMaintenanceService` は dependency graph / cache metadata の writeback と保守処理だけを担当する。JSON 出力指定時は `CLI Adapter` が `CompileResult` から versioned compile result object を生成し、標準出力にはその 1 object だけを出力する。
...
| バージョン | 日付 | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 0.1.28 | 2026-03-18 | 入力参照先を requirements v0.1.41 / domain model v0.1.39 へ更新し、bundle bootstrap 契約修正後の参照版数と同期 | Codex |
