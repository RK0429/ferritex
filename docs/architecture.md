...
| 最終更新日 | 2026-03-17 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.31, [domain_model.md](domain_model.md) v0.1.32 |
...
- commit 順序: `CommitBarrier` は `(passNumber, stageOrder, partitionId)` の total order で結果を適用する (`REQ-FUNC-031`, `REQ-FUNC-032`)。`stageOrder` は以下の順序に固定する:
...
| Typesetting Engine | 行分割、ページ分割、display math、フロート、脚注、目次/索引、参考文献リストのレイアウト | `PageBox`, `FloatPlacement`, `DocumentLayoutFragment`（差分コンパイルのパーティション統合用、`domain_model.md` §3.4 で定義） |
| Incremental Compilation | 依存グラフ、変更検知 (`REQ-FUNC-028`)、影響範囲判定、`DocumentPartitionPlanner` による stable `partitionId` と `PartitionLocator` 発行（セクション境界は `DocumentState.toc` の `TocEntry.DefinitionProvenance` から、ファイル間依存は `DependencyGraph` から導出する）、再利用/再構築統合、固定点反復、文書パーティション統合 | `RecompilationScope`, `CompilationMergePlan`, `PaginationMergeResult`, `CompilationSnapshot` |
...
| `PreviewSessionService` | preview session の発行・再発行・失効、`Preview Adapter` へ返す bootstrap payload の決定、`PreviewTarget` owner 管理、閲覧位置保存、PDF 更新通知、`ExecutionPolicy.previewPublication` に照らした publish 可否判定と再適用を行う |
...
| `PreviewTransport` | session ごとの PDF document endpoint と revision events endpoint を提供 | loopback のみへ bind し、`GET /preview/{sessionId}/document` / `WS /preview/{sessionId}/events` の endpoint を公開する。`GET /preview/{sessionId}/document` には `Cache-Control: no-store, no-cache, must-revalidate` を付与する。session bootstrap の HTTP 入口は `Preview Adapter` が担当し、`PreviewSessionService` が決定した payload を使って、bind / publish は同 service が `ExecutionPolicy.previewPublication` と `PreviewTarget` 一致を確認した場合にだけ実行する |
...
### 10.13 セキュリティ境界 (`REQ-NF-005`, `REQ-NF-006`)
...
> **注記**: `REQ-FUNC-032`（文書パーティション単位並列化、Should）の適合度関数は現時点では定義していない。本機能の実装着手時に、シングルスレッド実行との出力同一性とスループット改善を検証する適合度関数を追加する。
...
- `PreviewSessionService` は `Preview Adapter` から委譲された bootstrap request と preview publish の両方で `ExecutionPolicy.previewPublication` と `PreviewTarget` 一致を確認し、許可された場合だけ `PreviewTransport` に loopback bind 済み endpoint への配信を指示する
...
| 観点 | 状態 | 補足 |
| --- | --- | --- |
| 品質特性シナリオ定義 | 充足 | QA-01 〜 QA-09 に整理済み |
...
| 適合度関数の要件トレーサビリティ | 充足 | §10.3 差分コンパイル正確性に `REQ-FUNC-030` を紐づけ済み |
