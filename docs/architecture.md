...
| 最終更新日 | 2026-03-17 |
| ステータス | ドラフト |
| 入力 | [requirements.md](requirements.md) v0.1.22, [domain_model.md](domain_model.md) v0.1.24 |
...
graph LR
    ...
    CLI --> CompileSvc
    LSP --> OpenDocs
    LSP --> CompileSvc : background compile
    ...
    Assets --> Font
    Parser --> Bibliography
    Bibliography --> Typeset : via DocumentState.BibliographyState
    Parser --> Typeset
    Parser --> Graphics
    Parser --> Incremental
    Typeset --> Graphics
    ...
    Incremental --> Cache
    Cache --> Incremental : corruption notification
    Policy --> Files
    Policy --> Tools
    Parser --> Files
    Assets --> Files
...
補足: `LSP --> CompileSvc` は LSP がバックグラウンドコンパイルまたは diagnostics refresh を要求する場合の runtime 上の協調関係を表す（§7.3 ステップ 3 参照）。LSP の read path 自体は `OpenDocumentStore` / `LiveAnalysisSnapshotFactory` を経由し、active compile job の完了を待たない。
