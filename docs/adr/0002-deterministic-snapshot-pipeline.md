...
| 著者 | ferritex team |
| 関連ADR | ADR-0001 |
| 反映先 | [architecture.md](../architecture.md) §3.1 採用パターン（commit 順序）, §5.1 Compilation Core（CompilationSnapshot）, §7.1 `compile`, §7.3 `lsp`, §10.12 パイプライン並列化, §10.13 文書パーティション単位並列化, §11.4 性能設計; [domain_model.md](../domain_model.md) §6.1 判断記録（ADR-0002）, §6.28 判断記録（ADR-0002）, §6.29 判断記録（ADR-0002） |
...
- `CompilationSnapshot`: compile pipeline の並列ステージが参照する、job/pass 状態の frozen read-only projection
- `LiveAnalysisSnapshot`: LSP が参照する、`OpenDocumentBuffer` と Stable Compile State から生成される別契約の immutable projection

ここでいう Stable Compile State とは、最新の成功した `CommitBarrier` 完了時点で確定した `CompilationSession` / `DocumentState` の frozen read-only projection を指し、worker-local な未 commit 状態や失敗 pass の部分結果は含まない。
...
compile の並列ステージは `CompilationSnapshot` だけを読み、job-scope の更新は順序付き barrier でまとめて commit する。`CompilationSnapshot` と Stable Compile State は mutable aggregate を直接共有せず、凍結済み view として構築する。LSP は別契約の `LiveAnalysisSnapshot` を参照し、mutable な `CompilationSession` を直接保持しない。
...
- `CompilationSnapshot` と `LiveAnalysisSnapshot` を同一型のまま実装したり、mutable aggregate をそのまま保持したりすると不変条件が曖昧になる
