...
- **入力**: ファイルパスを含むトークン
- **処理**:
  - `TEXINPUTS` 環境変数および kpathsea 互換のパス探索
  - `\include` のガード処理（`.aux` ファイルの分離）
  - ファイルのネスト深度管理
- **出力**: 読み込んだファイルのトークンストリームを現在のストリームに挿入
- **例外**: ファイル未発見時にエラーを報告（`\InputIfFileExists` の場合は偽ブランチを実行）
- **受け入れ基準**:
  - Given `TEXINPUTS` に複数ディレクトリが設定された環境, When `\input{myfile}` を実行, Then パス順序に従ってファイルが検索・読み込まれる
- **優先度**: Must
- **出典**: ユーザー明示
...
- **説明**: 指定されたフォント名からフォントファイルの実体パスを解決する
- **入力**: フォント名またはファイル名
- **処理**:
  - `TEXMF` ツリーの探索（kpathsea 互換ロジック）
  - `ls-R` データベースまたはファイルシステム探索
  - フォントマップファイル（`.map`）の参照
- **出力**: フォントファイルの絶対パス
- **例外**: フォント未発見時にエラーを報告し、代替フォント（Computer Modern）へフォールバック
- **受け入れ基準**:
  - Given TeX Live がインストールされた環境, When `cmr10` を検索, Then 正しい TFM ファイルパスが返される
- **優先度**: Must
- **出典**: ユーザー明示
...
- **受け入れ基準**:
  - Given `\documentclass{article}` で始まる標準的な論文, When コンパイル, Then pdfLaTeX と同等のレイアウトで PDF が生成される
...
- **説明**: TeX の `\openin`, `\openout` によるファイルアクセスを、プロジェクトディレクトリとシステム TeX ディレクトリに制限する
- **定量基準**: プロジェクトディレクトリ外（`TEXMF` ツリーを除く）への書き込みが発生する経路がゼロ
- **優先度**: Must
- **出典**: エージェント推測（pdfLaTeX の `openout_any = p` 相当のセキュリティモデル）
...
- **定量基準**: `cargo install ferritex` または単一バイナリのダウンロードで利用開始可能。TeX Live のインストールは前提条件とする（フォント・パッケージの参照に必要）
- **優先度**: Should
...
| 2   | tikz/pgf の互換レベル。tikz は内部で膨大な TeX マクロを使用しており、完全互換には相当な工数が必要。対応する tikz のサブセット（基本図形のみ等）を明確化する必要がある       | REQ-FUNC-023 | 開発者  |
...
| 6   | 対応する TeX Live のバージョン範囲。kpathsea 互換のファイル探索が、どのバージョンの TeX Live ディレクトリ構造を前提とするか                          | REQ-FUNC-019 | 開発者  |
| 7   | `--shell-escape` 時の許可コマンドリスト。pdfLaTeX の restricted shell escape 相当の機能を実装するか、全コマンド許可のみとするか             | REQ-NF-005   | 開発者  |
