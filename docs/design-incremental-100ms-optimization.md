...
| Version | 0.1.0 |
| Date | 2026-04-04 |
| Status | Achieved（release-mode 5-run median: 66ms no-ref / 70ms with-ref、2026-04-05 再計測） |
...
- **評価**: **100ms 達成に必須だった。Option B と組み合わせて実装済み**
...
**制限**: page payload reuse は `Cached` / `BlockReuse` partition のページだけが対象で、`SuffixRebuild` / `FullRebuild` partition は全ページ再 render する。external image / embedded PDF graphic を含むページと fallback partition 文書では safety-first で reuse を無効化する。Step 3 は render 側 frontier を閉じた。`REQ-NF-002` の 100ms 達成はその後の Cache I/O Final（binary cache + CacheIndexSnapshot + background store + cached subtree fast path）で実現された。
...
| **Stage timing 計測なしでの最適化着手** | 緩和済み | — | Step 0 + 収束ループ分析完了により `StageTiming` / `pass_count` 計装が稼働中。baseline で cache I/O 78%・`pass_count=1` を確認。Cache I/O Final で REQ-NF-002 達成済み（70ms with-ref、2026-04-05 再計測） |
...
**次 frontier（解決済み）**: Round 2 時点では cache I/O 252ms（68.3%）が支配的だった。Round 3（zero-clone）を経て、最終的に Cache I/O Final（binary cache + CacheIndexSnapshot + BackgroundCacheWriter + cached subtree fast path）で **REQ-NF-002 達成**（70ms with-ref、2026-04-05 再計測）。
...
- **computation は 113ms（32.5%）** で Round 3 時点では 100ms 目標を単独で超過していた。その後の cached subtree fast path + background store により、computation の一部（source_tree_load）も削減され、総合で 100ms 未満を達成
- **3 ラウンドの累積効果（Round 3 時点）**: cache I/O は 460ms → 231ms（-49.8%）と半減に近い。total は 588ms → 348ms（-40.8%）。その後の Cache I/O Final で 70ms（-88.1%）まで改善し、**REQ-NF-002 達成**

**Cache I/O Final（解決済み）**: Round 3 時点では cache I/O 231ms（66.4%）が支配的で「構造を変えない最適化」は一巡していた。以下の構造的変更により **REQ-NF-002 を達成**した:
...
**最終計測結果**: release-mode hot incremental 5-run median: **66ms**（no-ref）/ **70ms**（with-ref）。baseline 588ms → Round 3 348ms（-40.8%）→ Final 70ms（-88.1%）の累積改善で 100ms 未満を達成（2026-04-05 再計測）。
