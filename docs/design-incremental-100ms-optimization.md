...
現行パイプラインで 100ms を超過する原因を、pipeline stage ごとに整理する。なお、stage 別の定量計測は未実施であり、以下はコード構造からの定性分析である。
...
- **計装の先行実装が必須**: stage 別 timing がない状態で最適化を始めるのは危険。parse/render/cache の支配率がコードからの定性推定しかないため、Step 0 として計装を入れる
...
### Step 0: Pipeline 計装（前提作業）

`CompileJobService` に stage timer と counters を追加し、`FTX-BENCH-001` 5-run median で stage 別コスト分布を可視化する。

| 計測対象 stage | 対象コード |
|---|---|
| source tree lookup/load | `compile_job_service.rs` source tree construction |
| parse + macro expansion | `compile_job_service.rs:2112` `parse_document_with_cross_references` |
| typeset (per-partition) | `typesetting/api.rs` `build_vlist` / `paginate_vlist` |
| PDF render | `pdf/api.rs:232` |
| cache store/load | `compile_cache.rs` serialize/deserialize |

**成果物**: stage 別 median timing。以降の Step の優先順位を定量的に判断する根拠となる。
...
| **Stage timing 計測なしでの最適化着手** | 中 | — | Step 0 を必須の前提作業とし、定量データなしでの Step 1 以降の着手を禁止する |
...
parse が全文のまま残るが、1000-section 文書でも parse 自体は typeset/render と比較して軽量である可能性が高い（Step 0 の計測で確認）。parse が支配的と判明した場合のみ Step 4 に進む。

Step 4 なしで 100ms を達成できるかは Step 0 の計測結果に依存するが、typeset と render の sub-document 化（Step 2 + 3）で大半の固定費を除去できる見込みは高い。
