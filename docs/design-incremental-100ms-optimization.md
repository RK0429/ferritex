...
| Version | 0.1.0 |
| Date | 2026-04-04 |
| Status | Proxy evidence only（release-mode internal proxy 5-run median: 66ms no-ref / 70ms with-ref、2026-04-05 再計測）。Issue #45 records current release-build public CLI warm incremental median 8.13s, so these figures are not a public CLI latency guarantee |
...
Wave 1（Incremental Performance Evidence）では `FTX-STRESS-2000` 1000-section staged input に対し、warm incremental compile 15.550s vs full `--no-cache` 28.614s（1.84× speedup）を point-in-time 計測した。これは incremental compile **機構**の有効性を実証したものであり、100ms 目標とは 2 桁以上の乖離がある。canonical `FTX-BENCH-001` は fixed asset bundle 前提の 100-page full-compile fixture として別管理である。

### 1.3 ボトルネック分析

現行パイプラインで 100ms を超過する原因を、pipeline stage ごとに整理する。Step 0 の計装（`StageTiming`）により stage 別の定量計測基盤は整備済みであり、以下は Step 3 完了後のコード構造と WU-5 再 profiling 結果に基づく分析である。canonical `FTX-BENCH-001` と `FTX-STRESS-2000` stress benchmark の両方で定量データを継続確認し、追加最適化の優先順位を判断する。

| Stage | 現状の問題 | 根拠 |
|---|---|---|
| **依存検出** | Step 1 で watcher/scheduler からの `changed_paths` fast path、split cache、watcher backend 抽象化は導入済み。ただし empty hint 時は依存グラフ上の全 node 走査に fallback する | `CompileCache::detect_changes()` |
| **Parse** | source subtree reuse は I/O 削減のみ。expanded 全文を毎回 parser に渡し直す | `expand_inputs()` |
| **Typeset** | Step 2 の block checkpoint reuse は benchmark 条件で有効化されており、WU-5 再 profiling では `FTX-STRESS-2000` 1000-section staged input の 1 段落変更時に `cached=999 partition`, `suffix_rebuild=1 partition`, `full_rebuild=0 partition` を記録した。変更対象 partition は `reuse=SuffixRebuild, suffix=2/4, fallback=None` で処理され、以前の `SuffixValidationFailed` 起因の full rebuild fallback は解消された。`StageTiming.pass_count` 計装後の with-ref benchmark では typeset median は 57ms（9.7%）で、主要律速ではないことが確認された | `StageTiming.typeset_partition_details`, `CompileJobService::compile_one_pass()` |
| **Cross-ref 収束** | `\pageref` 存在時は partial typeset 無効化。2〜3 pass 回り得る | `cached_cross_reference_seed`, `partial_typeset_available` |
| **PDF 出力** | Step 3 で per-page payload reuse は実装済み。ただし再利用対象は `Cached` / `BlockReuse` partition のページに限られ、fallback partition を含む文書や XObject-backed page は safety-first で再 render する | `reusable_page_payloads_for_render()` |
| **Cache 保存** | split cache（現在の format は v7）で monolithic JSON は解消したが、`index.json` と変更 partition blob の serialize/deserialize は依然として発生する | `CompileCache::store()`, `BackgroundCacheWriter` |

**構造的制約**: typeset と render は sub-document 粒度まで縮小できているが、parse と一部 fixed-cost stage は依然として全文・全 partition 相当の処理を含む。`StageTiming.pass_count` 計装と `FTX-STRESS-2000` 1000-section staged input の 5-run benchmark により、incremental compile の baseline は no-ref 520ms / with-ref 588ms、pass_count はいずれも 1 と確認された。Round 1（delta write）で with-ref median は 505ms、Round 2（dirty-tracking store + warm-cache lookup）で 369ms、Round 3（zero-clone warm-cache）で 348ms まで低下した。その後の Cache I/O Final（v8 binary cache + CacheIndexSnapshot + BackgroundCacheWriter + cached subtree fast path）で release-mode internal proxy 5-run median **66ms**（no-ref）/ **70ms**（with-ref）を記録した（2026-04-05 再計測）。ただし Issue #45 records current release-build public CLI warm incremental median **8.13s** for the UX reproduction fixture, so the 66ms/70ms proxy figures are not a public CLI latency guarantee. 旧来の 66.8s 計測値は `--no-cache` 初期 compile を同一 run に含む debug 条件のため hot incremental path の比較基準としては不適切である点は変わらない。

### 1.4 収束ループ分析（2026-04-05）

`StageTiming.pass_count` 計装により、`\ref` + `\tableofcontents` を含む `FTX-STRESS-2000` 1000-section staged input に対する incremental compile の収束パス数を計測した。
...
**結論**: baseline では cache I/O（cache_store + cache_load = 460ms、78%）が最優先ターゲットだった。Round 1（delta write）で with-ref 5-run median は 505ms、cache I/O は 389ms（77.0%）まで低下した。Round 2（dirty-tracking store + warm-cache lookup）で cache_store は 245ms → 101ms（-58.8%）に劇的改善し、total は 369ms、cache I/O は 252ms（68.3%）まで低下した。Round 3（zero-clone warm-cache）で cache_load は 151ms → 139ms（-7.9%）、cache_store は 101ms → 92ms（-8.9%）に改善し、total は 348ms、cache I/O は 231ms（66.4%）まで低下した。dominant stage は引き続き cache_load 139ms（40.0%）である。cache I/O が引き続き最優先 frontier であり、binary serialization format（bincode 等）と lazy partition load が次の候補である。ただし computation 単独でも 113ms と 100ms 目標を超過しており、cache I/O 削減だけでは不十分。Step 4（incremental parse）は parse が 22ms（6.3%）に留まるが、source_tree_load 23ms（6.6%）と合わせた fixed-cost 45ms の削減も検討対象だった。最終的に Cache I/O Final（v8 binary cache + CacheIndexSnapshot + BackgroundCacheWriter + cached subtree fast path）で release-mode internal proxy 5-run median **66ms**（no-ref）/ **70ms**（with-ref）を記録した。ただしこれは `FTX-STRESS-2000`-class proxy 上の内部計測であり、Issue #45 の public CLI UX reproduction fixture では **8.13s** が観測されている。
...
- **リスク**: 中。footnote/float の継続状態の正確な保存・復元が最大の技術リスク。`append_cached_footnote_lines()` 周辺の footnote merge が要注意
...
- **評価**: **proxy 100ms 未満の内部計測に必須だった。Option B と組み合わせて実装済み**
...
| Option | 効果 | リスク | 実装コスト | 単独で proxy 100ms 未満 | 推奨順序 |
|---|---|---|---|---|---|
...
**A → B → C は実装済みで、cache I/O Round 1（delta write）+ Round 2（dirty-tracking store + warm-cache lookup）+ Round 3（zero-clone warm-cache）が完了した。** suffix rebuild 改善と収束ループ分析に加え、Round 1 では compact JSON serialization・`partition_hashes` 追跡・未変更 blob write skip を導入し、Round 2 では clean partition の serialization スキップ（dirty-tracking）と前回 lookup の blob 再利用（warm-cache lookup）を導入し、Round 3 では `WarmPartitionCache` を flat map + move ベースへ再設計して partition blob の clone 経路を除去した。with-ref 5-run median は 588ms → 505ms → 369ms → 348ms、cache I/O は 460ms（78%）→ 389ms（77.0%）→ 252ms（68.3%）→ 231ms（66.4%）と段階的に低下した。cache_store は 289ms → 92ms（-68.2%）、cache_load は 171ms → 139ms（-18.7%）に改善したが、cache I/O 231ms は依然支配的で computation 113ms と合わせ 348ms。その後の Cache I/O Final（v8 binary cache + CacheIndexSnapshot + BackgroundCacheWriter + cached subtree fast path）で release-mode internal proxy 5-run median **66ms**（no-ref）/ **70ms**（with-ref）を記録した（2026-04-05 再計測）。D（incremental parse）は internal proxy 上では不要だったが、Issue #45 の public CLI UX reproduction fixture では **8.13s** が観測されているため、この値を public CLI latency guarantee として扱わない。
...
2. **段階的な効果検証**: Step 0 + 収束ループ分析で `pass_count=1` / cache I/O 78% を確認し、Round 1（delta write）で 505ms / 389ms、Round 2（dirty-tracking + warm-cache）で 369ms / 252ms、Round 3（zero-clone warm-cache）で 348ms / 231ms まで低下した。cache_store は 92ms、cache_load は 139ms まで改善したが、JSON serialization overhead が残存しており clone 除去だけでは限界に近い。次段階は binary format + lazy load による serialization 自体の置換が必要
...
- **計装の先行実装（Step 0 + 収束ループ分析完了済み）**: `StageTiming` と `pass_count` により stage 別 timing と収束回数の取得基盤が整った。baseline では with-ref benchmark の total 588ms・cache I/O 78% を確認し、Round 1（delta write）後は total 505ms・cache I/O 389ms（77.0%）、Round 2（dirty-tracking + warm-cache）後は total 369ms・cache I/O 252ms（68.3%）、Round 3（zero-clone warm-cache）後は total 348ms・cache I/O 231ms（66.4%）・`pass_count=1` を確認済み
- **benchmark の固定構成**: canonical `FTX-BENCH-001` は fixed asset bundle 前提の 100-page academic fixture を使用する。incremental stress benchmark は `FTX-STRESS-2000` の staged input を使用し、monolithic single-file ではなく partition entry file 単位への staged 変換を前提とする（Wave 1 の設計判断を踏襲）
- **preamble 変更は full fallback を許容**: preamble 変更は proxy timing scope 外。本文 1 段落変更が対象
...
**有効化状態**: benchmark 条件で `BlockLevel` scope が生成・適用されることを確認済み。`compile_cache.rs` が block checkpoint を持つ partition で `LocalRegion` → `BlockLevel` に昇格し、`compile_job_service.rs` の `partial_typeset_available` が `BlockLevel` を受理し、`TypesetterReusePlan::create()` が `primary_input_changed` ガードをバイパスする。WU-5 再 profiling では staged `FTX-STRESS-2000` の変更 partition が `SuffixRebuild` として処理され、従来の `SuffixValidationFailed` fallback は再現しなかった。baseline の with-ref 5-run median 588ms / `pass_count=1` から、Round 1 の 505ms、Round 2 の 369ms / `pass_count=1` まで低下した。Step 2 の改善後も主要 frontier は cache I/O であり、multi-pass guard は不要と判明した。次の作業は split cache の read/write I/O 最適化である（§7 参照）。
...
**制限**: page payload reuse は `Cached` / `BlockReuse` partition のページだけが対象で、`SuffixRebuild` / `FullRebuild` partition は全ページ再 render する。external image / embedded PDF graphic を含むページと fallback partition 文書では safety-first で reuse を無効化する。Step 3 は render 側 frontier を閉じた。その後の Cache I/O Final（binary cache + CacheIndexSnapshot + background store + cached subtree fast path）は internal proxy 100ms 未満の計測値に寄与したが、Issue #45 の public CLI UX reproduction fixture では **8.13s** が観測されている。
...
| リスク | 影響度 | 発生可能性 | 緩和策 |
|---|---|---|---|
| **Footnote/float 継続状態の不整合** | 高 | 中 | `append_cached_footnote_lines()` 周辺の footnote merge を集中テスト。checkpoint に footnote queue 状態を含める |
...
| **Stage timing 計測なしでの最適化着手** | 緩和済み | — | Step 0 + 収束ループ分析完了により `StageTiming` / `pass_count` 計装が稼働中。baseline で cache I/O 78%・`pass_count=1` を確認。Cache I/O Final で internal proxy 70ms with-ref を記録（2026-04-05 再計測）。public CLI guarantee ではない |
...
**incremental benchmark 実測（2026-04-05）**: `incremental_stage_timing_5run_median`（no-ref）と `incremental_stage_timing_with_refs_5run`（`\ref` + `\tableofcontents`）で、`FTX-STRESS-2000` 1000-section staged input の 1 段落変更を 5-run 計測した。
...
**次 frontier（解決済み）**: Round 2 時点では cache I/O 252ms（68.3%）が支配的だった。Round 3（zero-clone）を経て、最終的に Cache I/O Final（binary cache + CacheIndexSnapshot + BackgroundCacheWriter + cached subtree fast path）で internal proxy 70ms with-ref を記録した（2026-04-05 再計測）。この値は public CLI latency guarantee ではない。

### Cache I/O Round 3: Zero-clone warm-cache (2026-04-05)

Round 2 では warm-cache lookup が clone overhead により逆効果だった（cache_load 144ms → 151ms、+4.9%）。Round 3 では `WarmPartitionCache` を flat map + move ベースへ再設計し、lookup/load/store/compile_job_service の clone 経路を除去した。`store_with_page_payloads` は新しい `partition_hashes` を返し、post-compile data から warm cache を再構築する。

| Stage | Median | % |
|---|---:|---:|
| `cache_load` | 139ms | 40.0% |
| `source_tree_load` | 23ms | 6.6% |
| `parse` | 22ms | 6.3% |
| `typeset` | 56ms | 16.1% |
| `pdf_render` | 12ms | 3.4% |
| `cache_store` | 92ms | 26.4% |
| `total` | 348ms | 100.0% |
| `cache I/O` | 231ms | 66.4% |
| `computation` | 113ms | 32.5% |

5-run 詳細:
- cache_load: 144ms, 138ms, 139ms, 139ms, 141ms
- cache_store: 92ms, 92ms, 92ms, 93ms, 93ms
- total: 352ms, 346ms, 347ms, 348ms, 350ms
- pass_count: 1, 1, 1, 1, 1

| Stage | Baseline | Round 1 | Round 2 | Round 3 | R2→R3 Delta | R2→R3 % | Baseline→R3 Delta | Baseline→R3 % |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `cache_store` | 289ms | 245ms | 101ms | 92ms | -9ms | -8.9% | -197ms | -68.2% |
| `cache_load` | 171ms | 144ms | 151ms | 139ms | -12ms | -7.9% | -32ms | -18.7% |
| `cache I/O total` | 460ms | 389ms | 252ms | 231ms | -21ms | -8.3% | -229ms | -49.8% |
| `total` | 588ms | 505ms | 369ms | 348ms | -21ms | -5.7% | -240ms | -40.8% |

**分析**:

- **cache_load が改善**（151ms → 139ms、-7.9%）。zero-clone 化により ~2000 partition blob の `Clone` が除去され、warm-cache hit 時の overhead が低下した。ただし改善幅は 12ms に留まり、clone overhead は全体の一部に過ぎなかった。read-side のコストの大部分は JSON deserialize + disk I/O（index.json + blob files）であり、clone 除去だけでは根本解決に至らない
- **cache_store も改善**（101ms → 92ms、-8.9%）。warm cache 再構築時の clone 除去と、post-compile data からの直接移動による効率化が寄与
- **computation は 113ms（32.5%）** で Round 3 時点では 100ms 目標を単独で超過していた。その後の cached subtree fast path + background store により、computation の一部（source_tree_load）も削減され、internal proxy total は 100ms 未満になった
- **3 ラウンドの累積効果（Round 3 時点）**: cache I/O は 460ms → 231ms（-49.8%）と半減に近い。total は 588ms → 348ms（-40.8%）。その後の Cache I/O Final で internal proxy は 70ms（-88.1%）まで改善したが、Issue #45 の public CLI UX reproduction fixture では **8.13s** が観測されている

**Cache I/O Final（解決済み）**: Round 3 時点では cache I/O 231ms（66.4%）が支配的で「構造を変えない最適化」は一巡していた。以下の構造的変更により internal proxy 70ms with-ref を記録した:

1. **v8 binary cache（bincode）**: JSON の serialize/deserialize overhead を根本排除。`index.bin` + `partitions/*.bin` 形式で cache_load・cache_store 双方を大幅改善。v7/v6 JSON fallback と stale `index.json` cleanup を実装
...
3. **BackgroundCacheWriter**: `store_background()` で partition blob の bincode serialization / disk write / cleanup を background thread に移動し、store latency を hot path から除外。`watch_runner` は `flush_cache()` で drain
...
**最終 proxy 計測結果**: release-mode internal proxy hot incremental 5-run median: **66ms**（no-ref）/ **70ms**（with-ref）。baseline 588ms → Round 3 348ms（-40.8%）→ Final 70ms（-88.1%）の累積改善で proxy 100ms 未満を記録した（2026-04-05 再計測）。Issue #45 records current release-build public CLI warm incremental median **8.13s** for the UX reproduction fixture; do not present the proxy result as a public CLI latency guarantee.
