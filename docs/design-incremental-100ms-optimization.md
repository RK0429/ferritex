...
`incremental_stage_timing_5run_median` により、1000-section staged `FTX-BENCH-001` の 1 段落変更に対する `StageTiming` の 5-run median を取得した。

| Stage | Median | Runs |
|---|---:|---|
| `cache_load` | 139ms | 140ms, 140ms, 139ms, 139ms, 139ms |
| `source_tree_load` | 22ms | 23ms, 23ms, 22ms, 22ms, 22ms |
| `parse` | 17ms | 17ms, 17ms, 16ms, 17ms, 17ms |
| `typeset` | 56ms | 55ms, 56ms, 56ms, 55ms, 56ms |
| `pdf_render` | 9ms | 9ms, 9ms, 9ms, 9ms, 9ms |
| `cache_store` | 250ms | 261ms, 250ms, 250ms, 253ms, 244ms |
| `total` | 496ms | 507ms, 496ms, 495ms, 499ms, 490ms |

instrumented stages 内の最大コストは `cache_store` であり、median 250ms、`StageTiming` total 496ms に対して **50.5%** を占める。したがって、instrumented stages の範囲では parse ではなく cache write が支配的である。

一方で、この `StageTiming` total は manual benchmark の wall-clock **66,782ms** と比べて約 134× 小さく、捕捉できているのは wall-clock の約 0.7% に過ぎない。これは、現状の `StageTiming` が計測していないパイプラインのオーケストレーション、特に `parse_document_with_cross_references` の反復呼び出しや cross-reference convergence loop の overhead が真の律速であることを示唆する。

この結果により、律速要因の解釈は「単一の instrumented stage」から「instrumented stages 外の convergence / orchestration overhead」へ更新される。次の推奨アクションは、`StageTiming` 合計 496ms と wall-clock 66,782ms の gap を埋めるため、cross-reference convergence loop の反復回数、各 pass 所要時間、`parse_document_with_cross_references` の総滞在時間を計測し、どの orchestration 層が 66.8s を消費しているかを特定することである。
