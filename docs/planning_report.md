...
| ID | 要件領域 | 深刻度 | 主な不足 |
|---|---|---|---|
| A | Incremental compilation (REQ-FUNC-027-030) | 高 | dependency graph、persistent cache、change detection、partial recompile merge は未実装 |
...
| # | タスク | 受入基準 |
|---|---|---|
| 1 | Dependency graph / change detection | 変更ファイル集合から再処理範囲を決定できる |
| 2 | Persistent cache / integrity check | warm compile で再利用が走り、cache 破損時は full compile fallback できる |
