...
- **残 Frontier**: `REQ-NF-002` Step 3/4（per-page payload reuse / incremental parse、設計文書: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)）。Wave 4 long-tail は multicol MVP 完了により解消（既知制限: region のページ跨ぎ未対応）
- **profiling 現況**: Step 2 block reuse は `BlockLevel` scope 生成・partial typeset gating・`primary_input_changed` バイパスにより benchmark 条件で有効化されているが、staged `FTX-BENCH-001` では typeset が依然支配的。suffix rebuild のスコープが十分に縮小されていない可能性があり、Step 3 着手前に typeset コスト内訳の追加分析が必要
- **直近の推奨**: (1) staged `FTX-BENCH-001` での Step 2 有効時の stage timing を計測し、typeset が支配的な原因（suffix 範囲の広さ、fallback 頻度等）を特定する。(2) その結果に基づいて Step 3（per-page payload reuse）に進むか、先に typeset 側の追加縮小を行うかを判断する
...
graph LR
    ...
    S2 --> S2A[Step 2 activation 確認 + typeset 分析]
    S2A --> S34[Step 3-4: render / parse]
    W4 --> W4R[Wave 4: multicol MVP ✓]
...
- **判断**: 実装の核心部分と parity evidence 接続が完了。REQ-NF-007 の 5 カテゴリ parity は全 pass。Wave 1〜3 に加え、REQ-NF-002 は Step 0 の計装、Step 1 の fixed-cost 削減（`changed_paths` fast path、v5 split cache、watcher backend 抽象化）、Step 2 の block checkpoint reuse（`RecompilationScope::BlockLevel`、checkpoint 生成、suffix rebuild、fallback 条件）まで完了し、typeset 側の sub-document reuse が導入された。Step 2 は benchmark 条件で `BlockLevel` scope が有効化されている（`CompileCache` が `LocalRegion` → `BlockLevel` 昇格、`partial_typeset_available` が `BlockLevel` 受理、`TypesetterReusePlan` が `primary_input_changed` バイパス）。ただし staged `FTX-BENCH-001` profiling では typeset が依然支配的であり、suffix rebuild スコープの縮小効果が期待より限定的である可能性がある。Wave 4 では bounded TikZ 改善、package compatibility MVP、multicol MVP が全て完了し、long-tail frontier は解消された（multicol の既知制限: region のページ跨ぎ未対応）。残りは `REQ-NF-002` Step 3/4（per-page payload reuse / incremental parse）に加え、typeset が支配的な原因の追加分析
- **直近の推奨**: Step 2 有効時の stage timing 内訳を staged `FTX-BENCH-001` で計測し、typeset が支配的な原因を特定した上で、Step 3 着手か typeset 側追加縮小かを判断する
