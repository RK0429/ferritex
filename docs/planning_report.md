...
- **直近の推奨**: (1) `StageTiming` の stage 別計測で 66.8s を支配している stage を特定する。(2) 支配 stage が parse 以外（例: pdf_render の fixed-cost、cache I/O）であれば、その stage の guard 条件または fixed-cost を先に最適化する。(3) parse が支配的と判明した場合のみ Step 4 に進む
...
- **直近の推奨**: manual benchmark（5-run median 66,782ms）により 100ms 大幅未達が確定した。次は `StageTiming` で支配 stage を特定し、parse が支配的と確認できた場合のみ Step 4 に進む。別 stage が支配的ならその最適化を先に行う
