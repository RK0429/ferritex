...
- **Step 1 の残差分**: なし
- **後続 Frontier**: Step 3・収束ループ分析・cache I/O delta write は完了済み。with-ref 5-run median は 505ms まで低下したが、cache I/O 389ms（77.0%）が依然支配的である。次の優先課題は cache_load 144ms を中心とする cache read-side 最適化（lazy load、binary serialization format）であり、async / deferred write はその次段で再評価する。Step 4（incremental parse）はその後に再評価する
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)
...
- **制限 / 未完境界**: reuse 対象は `Cached` / `BlockReuse` partition のページに限定され、`SuffixRebuild` / `FullRebuild` partition は全ページ再 render する。fallback partition 文書と XObject-backed page は safety-first で対象外。with-ref 5-run median 505ms / `pass_count=1` により `REQ-NF-002` は未達で、次は cache I/O 389ms（77.0%）の削減が必要
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)
...
- **REQ-NF-002 Cache I/O delta write 完了**。compact JSON serialization、`partition_hashes` 追跡、未変更 blob write skip を実装し、split cache write path の fixed cost を削減した
...
- **残 Frontier**: `REQ-NF-002` は with-ref 5-run median 505ms により 100ms 未達であることが確認された。残件は cache I/O 最適化（cache_store 245ms + cache_load 144ms = 389ms, 77.0%）であり、write 側の delta write は部分完了、次の主対象は cache_load を中心とする read-side 最適化である。Step 4（incremental parse）はその後に再評価する（設計文書: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)）。Wave 4 long-tail は multicol MVP 完了により解消（既知制限: region のページ跨ぎ未対応）
...
- **incremental benchmark 実測（2026-04-05）**: `incremental_stage_timing_with_refs_5run`（with-ref）を delta write 後に再実行し、hot incremental path の median 505ms、`pass_count` は `1, 1, 1, 1, 1` を確認した。no-ref 520ms は旧 baseline のままで、delta write 後の再計測は未実施
- **bottleneck 実測（2026-04-05）**: with-ref 5-run median の dominant stage は依然 `cache_store` 245ms（48.5%）だが、write 側は今回部分対応済みである。`cache_load` 144ms（28.5%）を合わせた cache I/O は 389ms（77.0%）で、cache I/O 内の次の未着手機会は read-side 最適化である。parse 22ms（4.4%）、typeset 55ms（10.9%）、pdf_render 12ms（2.4%）は次段の対象
- **直近の推奨**: (1) partition blob / index の lazy load を導入して `cache_load` を削る。(2) binary serialization format を検討して read/write 双方の encode/decode を詰める。(3) cache write の async / deferred 化を再評価し、その後に Step 4（incremental parse）の必要性を再判定する
...
graph LR
    ...
    S3 --> BM[Benchmark: with-ref 505ms ✓]
    BM --> CA[Convergence: pass_count=1 ✓]
    CA --> DW[Delta write: cache I/O改善 ✓]
    DW --> SX[Next: cache I/O optimize]
...
- **判断**: 実装の核心部分と parity evidence 接続が完了。REQ-NF-007 の 5 カテゴリ parity は全 pass。Wave 1〜3 に加え、REQ-NF-002 は Step 0 の計装、Step 1 の fixed-cost 削減（`changed_paths` fast path、v5 split cache、watcher backend 抽象化）、Step 2 の block checkpoint reuse（`RecompilationScope::BlockLevel`、checkpoint 生成、suffix rebuild、fallback 条件）、Step 3 の per-page payload reuse（`PageRenderPayload.stream_hash`、`CachedPagePayload`、cache v7、pre-rendered payload 注入、Guard 1 + Guard 2）、収束ループ分析（`pass_count=1` 確認）、および cache I/O delta write（compact JSON・`partition_hashes`・未変更 blob write skip）まで完了した。Step 3 は partitioned report fixture で `reused_pages=39` / `rendered_pages=1` と full compile byte-identical を確認している一方、fallback partition 文書と XObject-backed page では fail-closed で無効化している。WU-5 再 profiling では `cached=999 partition`, `suffix_rebuild=1 partition`, `full_rebuild=0 partition`, `reuse=SuffixRebuild, suffix=2/4, fallback=None` が記録され、従来の `SuffixValidationFailed` fallback は解消された。delta write 後の with-ref 5-run median は 505ms、cache I/O は 389ms（77.0%）であり、100ms 未達は残るが、残りは cache I/O 最適化であり multi-pass 対策ではない。Wave 4 では bounded TikZ 改善、package compatibility MVP、multicol MVP が全て完了し、long-tail frontier は解消された（multicol の既知制限: region のページ跨ぎ未対応）
- **直近の推奨**: with-ref benchmark で cache_store 245ms（48.5%）+ cache_load 144ms（28.5%）が引き続き支配的と確認された。write 側は部分対応済みのため、次は partition blob / index の lazy load、binary serialization format、必要なら cache write の async / deferred 化を進め、その後に Step 4（incremental parse）の必要性を再評価する
