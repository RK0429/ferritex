...
| REQ-NF-002 収束ループ分析 | `StageTiming.pass_count` 計装、`\ref` 付き benchmark | **完了** — incremental compile は 1-pass（seed が有効）。bottleneck は cache I/O（78%）であり multi-pass ではない |
...
- **Step 1 の残差分**: なし
- **後続 Frontier**: Step 3 と収束ループ分析は完了済み。with-ref 5-run median 588ms / `pass_count=1` により、次の優先課題は cache I/O 最適化（serialize/deserialize 高速化または lazy load）である。Step 4（incremental parse）はその後に再評価する
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)
...
- **制限 / 未完境界**: reuse 対象は `Cached` / `BlockReuse` partition のページに限定され、`SuffixRebuild` / `FullRebuild` partition は全ページ再 render する。fallback partition 文書と XObject-backed page は safety-first で対象外。with-ref 5-run median 588ms / `pass_count=1` により `REQ-NF-002` は未達で、次は cache I/O 最適化が必要
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)
...
- **残 Frontier**: `REQ-NF-002` は with-ref 5-run median 588ms により 100ms 未達であることが確認された。残件は cache I/O 最適化（cache_store 289ms + cache_load 171ms = 460ms, 78%）であり、Step 4（incremental parse）はその後に再評価する（設計文書: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)）。Wave 4 long-tail は multicol MVP 完了により解消（既知制限: region のページ跨ぎ未対応）
...
- **incremental benchmark 実測（2026-04-05）**: `incremental_stage_timing_5run_median`（no-ref）と `incremental_stage_timing_with_refs_5run`（with-ref）で hot incremental path を 5-run 計測。median は no-ref 520ms / with-ref 588ms、`pass_count` は両方とも `1, 1, 1, 1, 1`
- **bottleneck 実測（2026-04-05）**: with-ref 5-run median の dominant stage は cache_store 289ms（49.2%）で、cache_load 171ms（29.1%）を合わせた cache I/O が 460ms（78%）を占める。parse 23ms（3.9%）、typeset 57ms（9.7%）、pdf_render 12ms（2.0%）は次段の対象
- **直近の推奨**: (1) split cache の serialize/deserialize を高速化する。(2) partition blob / index の lazy load を導入して cache_load を削る。(3) cache I/O 改善後に Step 4（incremental parse）の必要性を再判定する
...
graph LR
    ...
    S3 --> BM[Benchmark: with-ref 588ms ✓]
    BM --> CA[Convergence: pass_count=1 ✓]
    CA --> SX[Next: cache I/O optimize]
