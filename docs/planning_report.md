...
| ID | 要件領域 | 最終状態 |
|---|---|---|
| A | Incremental compilation (REQ-FUNC-027-030) | **全完了**。`REQ-NF-002` の 100ms 閾値は 1000-section staged input（`FTX-STRESS-2000`-class proxy、canonical `FTX-BENCH-001` 100-page fixture ではない）の 5-run median 66ms / 70ms で proxy clear 済み・canonical 計測は reliability-limited。Wave 1 correctness evidence + Step 0〜3 + Cache I/O Final まで完了 |
...
| 要件 | 対象 fixture | 判定 | 現状 | 将来タスク |
|---|---|---|---|---|
| REQ-NF-001 (フルコンパイル < 1.0s) | `FTX-BENCH-001`（100-page 学術論文 fixture、`crates/ferritex-bench/fixtures/bench/ftx_bench_001.tex`） | NON-BLOCKER (docs) | `bench_full_profile.rs` の `full_bench_docs_protocol_median_and_timing_proof` が jobs=1/jobs=4 の中央値を計測しログ出力。CI 環境のばらつきによる flaky リスクを考慮し assert は省略 | CI 安定環境での assert 追加を検討 |
| REQ-NF-001a (pdfLaTeX 比 50x) | `FTX-BENCH-001`（100-page 学術論文 fixture） | NON-BLOCKER (defer) | `requirements.md` §5 未確定事項 #2 で管理済み。ローカル計測では 54.89x（LTO jobs=4、2026-04-05） | pdfLaTeX インストール前提の比較テスト |
| REQ-NF-003 (メモリ < 1 GiB) | Heavy partition fixture `crates/ferritex-bench/fixtures/corpus/partition-article/heavy_sections_independent.tex`（`FTX-BENCH-001` ではない） | NON-BLOCKER (defer) | Should 優先度。上記 heavy fixture を `--jobs 16` で走らせると RSS が約 1133 MiB まで増える観測があり、高並列時は 1 GiB を超えうる。`FTX-BENCH-001` 100-page fixture に対する観測ではない。短期対応として `--jobs` help に peak RSS 注意書きを追加済み | `jemalloc stats_print` や `/proc/self/status` VmRSS での計測テスト追加 |
| REQ-NF-004 (LSP 応答速度) | `FTX-LSP-BENCH-001`（100-page warm trace、`crates/ferritex-cli/tests/bench_lsp_latency.rs::ftx_lsp_bench_001_warm_trace_meets_req_nf_004_thresholds`） | **達成（Complete）** | minimal document 3 テストに加え、`FTX-LSP-BENCH-001` 準拠の 100 ページ warm trace が 5 回測定の中央値を assert する。release build の実測中央値（2026-04-19）は diagnostics 35.69ms / completion 124.88µs / definition 129.25µs で、全閾値（500ms / 100ms / 200ms）を大幅に下回る。Warm state は (1) `ferritex compile` による full compile、(2) `ferritex lsp` + `initialize`、(3) `textDocument/didOpen` 後の初回 `publishDiagnostics` 待機、の 3 ステップを再現する。実行コマンド: `cargo test --release -p ferritex-cli --test bench_lsp_latency -- --ignored ftx_lsp_bench_001_warm_trace_meets_req_nf_004_thresholds`。`#[ignore]` ゲートのため通常 `cargo test` には載せず、REQ-NF-004 エビデンス取得時に明示的に起動する | なし |
| REQ-NF-010 (エラーメッセージ品質) | N/A | NON-BLOCKER (docs) | `Diagnostic` 構造体（`ferritex-core/src/diagnostics/diagnostic.rs`）が severity / file / line / message / context / suggestion を持ち、`Display` impl が file:line:message 形式で出力。TikZ パーサー、shell escape 等で使用 | 全エラーパスの構造化診断網羅テスト、preview session エラー応答フォーマット検証 |
...
| REQ-NF-002 Cache I/O Final: Binary cache + CacheIndexSnapshot + background store | v8 binary cache（bincode `index.bin` + `partitions/*.bin`）、`CacheIndexSnapshot` による warm lookup hot path 最適化、`BackgroundCacheWriter` で store を hot path から除外、cached subtree fast path で unchanged child の resolve/load を迂回 | **完了 — `REQ-NF-002` 100ms 閾値を proxy 計測で clear（canonical 計測は reliability-limited）**。実際の計測 fixture は 1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_5run_median` / `incremental_stage_timing_with_refs_5run` 生成）で、`REQ-NF-002` が canonical fixture として定義する `FTX-BENCH-001` 100-page fixture での計測ではない。release-mode hot incremental 5-run median: **66ms**（no-ref）/ **70ms**（with-ref）、いずれも 100ms 未満。canonical `FTX-BENCH-001` による formal validation は未実施。`ferritex-cli` 側の `partition_bench_multisecond_speedup_evidence` 等の pre-existing test failure は `REQ-NF-002` とは無関係であり、別トラックで対応 |
...
- **制限 / 未完境界**: reuse 対象は `Cached` / `BlockReuse` partition のページに限定され、`SuffixRebuild` / `FullRebuild` partition は全ページ再 render する。fallback partition 文書と XObject-backed page は safety-first で対象外。Round 3 時点では with-ref 5-run median 348ms / `pass_count=1` で `REQ-NF-002` の 100ms 閾値には未達だったが、その後の binary cache（v8 bincode）+ `CacheIndexSnapshot` + `BackgroundCacheWriter` + cached subtree fast path により **70ms（with-ref、1000-section staged input = `FTX-STRESS-2000`-class proxy、2026-04-05 再計測）** まで改善し、`REQ-NF-002` の 100ms 閾値は proxy 計測で clear した。canonical `FTX-BENCH-001` 100-page fixture での計測は未実施のため、`REQ-NF-002` は reliability-limited 扱いで保持する
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)
...
- **後続 Frontier（解決済み）**: Round 3 時点では cache I/O 231ms（66.4%）が支配的で「構造を変えない最適化」は一巡していた。その後、binary cache（v8 bincode）+ `CacheIndexSnapshot` + `BackgroundCacheWriter` + cached subtree fast path の構造的変更により、1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_*` tests）での release-mode hot incremental 5-run median は **66ms**（no-ref）/ **70ms**（with-ref）まで低下し、**`REQ-NF-002` の 100ms 閾値を proxy 計測で clear**（canonical `FTX-BENCH-001` 100-page fixture での計測は未実施のため reliability-limited 扱い）
- **設計文書**: [design-incremental-100ms-optimization.md](design-incremental-100ms-optimization.md)

### REQ-NF-002 Cache I/O Final: Binary cache + CacheIndexSnapshot + background store — 完了（proxy 計測で 100ms 閾値 clear、canonical 計測は reliability-limited）
...
- **計測 fixture（proxy）**: 1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_5run_median` / `incremental_stage_timing_with_refs_5run` がテスト内で生成する入力）。`REQ-NF-002` が canonical fixture として定義する `FTX-BENCH-001` 100-page 学術論文 fixture（`docs/requirements.md` §4.1 REQ-NF-002）での計測ではない
- **計測結果**: release-mode hot incremental 5-run median: **66ms**（no-ref）/ **70ms**（with-ref）。いずれも 100ms 未満であり、proxy 計測では `REQ-NF-002` の 100ms 閾値を clear している
- **信頼性限定**: canonical `FTX-BENCH-001` 100-page fixture による formal validation は未実施であり、上記 proxy 計測結果は point-in-time な reliability-limited indicator として保持する。`REQ-NF-002` canonical 計測は別トラックの open item として残る
...
- **REQ-NF-002 Cache I/O Final 完了（proxy 計測で 100ms 閾値 clear、canonical 計測は reliability-limited）**。v8 binary cache（bincode `index.bin` + `partitions/*.bin`）、`CacheIndexSnapshot` による warm lookup hot path 最適化、`BackgroundCacheWriter` で store を hot path から除外、cached subtree fast path で unchanged child の resolve/load を迂回。**計測 fixture は 1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_*` tests）であり、`REQ-NF-002` canonical fixture である `FTX-BENCH-001` 100-page 学術論文 fixture での計測ではない**。release-mode hot incremental 5-run median は **66ms**（no-ref）/ **70ms**（with-ref）で proxy 計測として 100ms 未満を維持している。canonical `FTX-BENCH-001` での formal validation は未実施のため、`REQ-NF-002` は reliability-limited 扱い。`ferritex-cli` 側の pre-existing test failure は別トラック
...
- **REQ-NF-002 proxy 計測で 100ms 閾値 clear（canonical 計測は reliability-limited）**: binary cache（v8 bincode）+ `CacheIndexSnapshot` + `BackgroundCacheWriter` + cached subtree fast path の構造的変更により、release-mode hot incremental 5-run median は **66ms**（no-ref）/ **70ms**（with-ref）まで低下し、1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_*` tests）の proxy 計測では 100ms 未満の定量基準を余裕をもって下回っている。ただし `REQ-NF-002` が canonical fixture として定義する `FTX-BENCH-001` 100-page 学術論文 fixture での計測は未実施のため、formal validation は reliability-limited な open item として保持する。baseline 588ms → Round 3 348ms（-40.8%）→ Final 70ms（-88.1%）の累積改善（いずれも proxy fixture 上の実測）。`ferritex-cli` 側の `partition_bench_multisecond_speedup_evidence` 等の pre-existing test failure は `REQ-NF-002` とは無関係であり、別トラックで対応する。Wave 4 long-tail は multicol MVP 完了により解消（既知制限: region のページ跨ぎ未対応）
...
- **incremental benchmark 実測（2026-04-05、Round 3 時点）**: `incremental_stage_timing_with_refs_5run`（with-ref、入力は `FTX-STRESS-2000` 1000-section staged input）を Round 3（zero-clone warm-cache）後に再実行し、hot incremental path の median 348ms、`pass_count` は `1, 1, 1, 1, 1` を確認した。Round 2 時は 369ms、Round 1（delta write）時は 505ms、baseline は 588ms
- **REQ-NF-002 最終 proxy 計測（2026-04-05）**: binary cache + `CacheIndexSnapshot` + `BackgroundCacheWriter` + cached subtree fast path の構造的変更後、release-mode hot incremental 5-run median: **66ms**（no-ref）/ **70ms**（with-ref）。計測 fixture は 1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_5run_median` / `incremental_stage_timing_with_refs_5run` 生成）であり、`REQ-NF-002` canonical fixture の `FTX-BENCH-001` 100-page 学術論文 fixture での計測ではない。baseline 588ms → 70ms（-88.1%）の累積改善で、**`REQ-NF-002` の 100ms 閾値を proxy 計測で clear**（canonical `FTX-BENCH-001` での formal validation は未実施、reliability-limited 扱い）
- **pre-existing test failure**: `ferritex-cli` 側の `partition_bench_multisecond_speedup_evidence` 等のベンチマーク統合テスト failure は REQ-NF-002 とは無関係な pre-existing issue であり、別トラックで対応する
...
graph LR
    ...
    FN --> DONE[REQ-NF-002 proxy 計測で 100ms clear: 66ms/70ms on FTX-STRESS-2000-class proxy, canonical FTX-BENCH-001 計測は reliability-limited ✓]
...
- **結果**: incremental 機構実証済み・parallel speedup evidence 取得済み・bundle archive CI 接続完了・Step 0 計装完了・Step 1 完了・Step 2 完了・Step 3 完了・Cache I/O Round 1（delta write）完了・Cache I/O Round 2（dirty-tracking + warm-cache）完了・Cache I/O Round 3（zero-clone warm-cache）完了・**Cache I/O Final（binary cache + CacheIndexSnapshot + background store）完了 — `REQ-NF-002` proxy 計測で 100ms 閾値 clear（canonical 計測は reliability-limited）**・Wave 4: TikZ + package compatibility + multicol MVP 完了
- **判断**: 実装の核心部分と parity evidence 接続が完了。REQ-NF-007 の 5 カテゴリ parity は全 pass。Wave 1〜3 に加え、`REQ-NF-002` は Step 0〜3、収束ループ分析、cache I/O Round 1〜3、および Final（binary cache + CacheIndexSnapshot + background store + cached subtree fast path）まで完了した。Final により release-mode hot incremental 5-run median は **66ms**（no-ref）/ **70ms**（with-ref）まで低下し、1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_*` tests）の proxy 計測では 100ms 未満の定量基準を下回っている。ただし `REQ-NF-002` が canonical fixture として定義する `FTX-BENCH-001` 100-page 学術論文 fixture での計測は未実施であり、formal validation は **reliability-limited** な open item として保持する（proxy と canonical は別 benchmark profile）。baseline 588ms → Round 3 348ms（-40.8%）→ Final 70ms（-88.1%）の累積改善（すべて proxy fixture 上）。`ferritex-cli` 側の pre-existing test failure は `REQ-NF-002` とは無関係であり、別トラックで対応する

### Warm Incremental Benchmark 実績 (REQ-FUNC-030) — Wave 1 完了

- **Status**: Wave 1 evidence established（REQ-FUNC-030 の correctness evidence を維持しつつ、stress benchmark timing は diagnostic として保持。`REQ-NF-002` はその後の cache I/O Final で proxy 計測で 100ms 閾値 clear、canonical 計測は reliability-limited）
...
- **構成**: `FTX-STRESS-2000` fixture（`crates/ferritex-bench/fixtures/bench/ftx_stress_2000_cycle.tex`、1000 `\section` staged input）を使い、cycle 900 で 1 段落変更を incremental compile。canonical `FTX-BENCH-001` は fixed asset bundle 付き 100-page full-compile fixture（`crates/ferritex-bench/fixtures/bench/ftx_bench_001.tex`）として別管理
...
- **REQ-NF-002 との関係**: Wave 1 stress benchmark は point-in-time diagnostic に留め、その後 Step 1〜3、cache I/O Round 1〜3、および Final（binary cache + CacheIndexSnapshot + background store + cached subtree fast path）を経て、`REQ-NF-002`（差分コンパイル中央値 100ms 未満）の 100ms 閾値は 1000-section staged input（`FTX-STRESS-2000`-class proxy、`incremental_stage_timing_*` tests、release-mode 5-run median: 66ms no-ref / 70ms with-ref）の proxy 計測で clear 済み。ただし `REQ-NF-002` canonical fixture である `FTX-BENCH-001` 100-page 学術論文 fixture での計測は未実施のため、formal validation は reliability-limited 扱い
...
- **構成**: heavy partition fixtures（`crates/ferritex-bench/fixtures/corpus/partition-book/heavy_chapters_independent.tex` / `crates/ferritex-bench/fixtures/corpus/partition-article/heavy_sections_independent.tex`、profile ID `FTX-PARTITION-BENCH-001`）を対象に、`--no-cache` + `--reproducible`、1 warmup + 5 measured runs、`--jobs=1` と `--jobs=4` を比較。`FTX-BENCH-001` 100-page fixture とは別 profile である
